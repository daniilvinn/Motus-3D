...
namespace Motus3D {
	...
	enum class MOTUS_API KeyCode : uint16_t {
	...
	{
	public:
		static bool KeyPressed(KeyCode code) { return s_Instance->KeyPressed_Impl(code); };
		static bool MouseButtonPressed(KeyCode code) { return s_Instance->MouseButtonPressed_Impl(code); };
	...
	protected:
		...
		virtual std::pair<int32_t, int32_t> MousePosition_Impl() = 0;

		// Mouse scrolled by Y axis
		// Returns -1 is scrolled down, 1 if scrolled up, 0 if not scrolled
		virtual float MouseScrolledY_Impl() = 0;
